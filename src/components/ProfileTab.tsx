...
export const ProfileTab: React.FC<ProfileTabProps> = ({ currentUser, onUpdateUser }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4 sm:px-6 space-y-8 animate-in fade-in duration-200">
      {/* User Header Card */}
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-stone-200/80 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div className="flex items-center gap-4 sm:gap-6">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs bg-orange-100 text-orange-800 font-semibold px-2 py-0.5 rounded-md">
                구글 연동 완료
              </span>
            </div>
            <p className="text-sm text-stone-500 font-medium mt-0.5">{currentUser.email}</p>
            <div className="flex items-center gap-4 mt-3 text-xs text-stone-600 font-medium">
              <span>선호 메뉴 <strong className="text-orange-600 font-bold">{currentUser.favoriteFoods.length}</strong>개</span>
              <span>•</span>
              <span>알레르기 <strong className="text-red-600 font-bold">{currentUser.allergies.length}</strong>건</span>
              <span>•</span>
              <span>최근 식사 <strong className="text-stone-800 font-bold">{currentUser.recentMeals.length}</strong>개</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2.5 sm:max-w-xs w-full">
          ...
          <div className="bg-emerald-50/90 border border-emerald-200/90 rounded-2xl px-3.5 py-2.5 flex items-center justify-between gap-2 text-xs shadow-xs">
            ...
            <button
              ...
              className="px-2.5 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-[11px] font-bold shadow-xs transition-colors flex items-center gap-1 flex-shrink-0 cursor-pointer"
            >
      ...
      {showSaveToast && (
        <div className="fixed bottom-6 right-6 z-50 bg-stone-900 text-white px-4 py-3 rounded-2xl shadow-xl border border-stone-700 flex items-center gap-2.5 text-xs font-semibold animate-in fade-in slide-in-from-bottom-2 duration-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
        {/* 1. Allergies */}
        <section className="bg-white rounded-3xl p-6 border border-stone-200/80 shadow-xs space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-base font-bold text-stone-900">알레르기 정보</h3>
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition-all ${
                    isSelected
                      ? 'bg-red-500 text-white shadow-xs scale-102 ring-2 ring-red-300'
          ...
          <div className="flex gap-2 pt-2">
            <input
              ...
              className="flex-1 px-3.5 py-2 text-xs rounded-xl border border-stone-200 focus:outline-none focus:ring-2 focus:ring-red-400 bg-stone-50"
            />
            <button
              onClick={handleAddCustomAllergy}
              className="px-3.5 py-2 rounded-xl bg-stone-800 text-white text-xs font-semibold hover:bg-stone-900 transition-colors flex items-center gap-1"
            >
          ...
          <div className="pt-2 border-t border-stone-100">
            <p className="text-xs font-semibold text-stone-400 mb-2">현재 등록된 알레르기:</p>
            {currentUser.allergies.length === 0 ? (
              <p className="text-xs text-stone-400 italic">등록된 알레르기가 없습니다.</p>
            ) : (
              <div className="flex flex-wrap gap-1.5">
                {currentUser.allergies.map((a) => (
                  <span
                    key={a}
                    className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-red-50 text-red-700 text-xs font-bold border border-red-200"
                  >
                    <span>{a}</span>
                    <button
        ...
        <div className="space-y-6">
          ...
          <section className="bg-white rounded-3xl p-6 border border-stone-200/80 shadow-xs space-y-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-base font-bold text-stone-900">선호 메뉴 (최애 음식)</h3>
                <p className="text-xs text-stone-500">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-3.5 py-2 text-xs rounded-xl border border-stone-200 focus:outline-none focus:ring-2 focus:ring-orange-400 bg-stone-50"
              />
              <button
                onClick={() => handleAddFavorite(newFavoriteInput)}
                className="px-3.5 py-2 rounded-xl bg-orange-600 text-white text-xs font-semibold hover:bg-orange-700 transition-colors flex items-center gap-1"
              >
                <Plus className="w-3.5 h-3.5" />
                <span>추가</span>
              </button>
            ...
            <div className="flex flex-wrap gap-1.5 pt-1">
              {currentUser.favoriteFoods.length === 0 ? (
                <p className="text-xs text-stone-400 italic">등록된 선호 메뉴가 없습니다. 좋아하는 음식을 추가해보세요!</p>
              ) : (
                currentUser.favoriteFoods.map((f) => (
                  <span
                    key={f}
                    className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-orange-50 text-orange-800 text-xs font-semibold border border-orange-200"
                  >
                    <span>{f}</span>
                    <button
            ...
            <div className="pt-2 border-t border-stone-100">
              <p className="text-[11px] text-stone-400 font-medium mb-1.5">인기 메뉴 빠른 추가:</p>
                ...
                {POPULAR_FOODS.slice(0, 8).map((food) => {
                  ...
                  return (
                    <button
                      ...
                      className="text-[11px] px-2 py-0.5 rounded-md bg-stone-100 text-stone-600 hover:bg-orange-100 hover:text-orange-800 transition-colors"
                    >
                      + {food}
                    </button>
                  );
          ...
          <section className="bg-white rounded-3xl p-6 border border-stone-200/80 shadow-xs space-y-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-base font-bold text-stone-900">비선호 메뉴</h3>
                <p className="text-xs text-stone-500">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-3.5 py-2 text-xs rounded-xl border border-stone-200 focus:outline-none focus:ring-2 focus:ring-stone-400 bg-stone-50"
              />
              <button
                onClick={() => handleAddDislike(newDislikeInput)}
                className="px-3.5 py-2 rounded-xl bg-stone-700 text-white text-xs font-semibold hover:bg-stone-800 transition-colors flex items-center gap-1"
              >
                <Plus className="w-3.5 h-3.5" />
                <span>추가</span>
              </button>
            ...
            <div className="flex flex-wrap gap-1.5 pt-1">
              {currentUser.dislikedFoods.length === 0 ? (
                <p className="text-xs text-stone-400 italic">비선호 메뉴가 없습니다.</p>
              ) : (
                currentUser.dislikedFoods.map((f) => (
                  <span
                    key={f}
                    className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-stone-100 text-stone-700 text-xs font-semibold border border-stone-200"
                  >
                    <span>{f}</span>
                    <button
      ...
      <section className="bg-white rounded-3xl p-6 sm:p-8 border border-stone-200/80 shadow-xs space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-lg font-bold text-stone-900">최근 먹은 메뉴 기록</h3>
              <p className="text-xs text-stone-500">
                지난 3일간 먹은 메뉴는 자동으로 추천 후보에서 제외됩니다 (단, 선호 메뉴는 예외로 유지).
              </p>
            </div>
          </div>
          <span className="text-xs bg-blue-50 text-blue-700 border border-blue-200 font-semibold px-3 py-1 rounded-full self-start sm:self-auto">
            3일 규칙 적용 중
          </span>
        </div>
        ...
        <form
          ...
          className="bg-stone-50 p-4 rounded-2xl border border-stone-200/70 flex flex-col sm:flex-row items-stretch sm:items-center gap-3"
        >
          <input
            type="text"
            required
            ...
            className="flex-1 px-3.5 py-2 text-xs rounded-xl border border-stone-200 bg-white focus:outline-none focus:ring-2 focus:ring-blue-400"
          />
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="px-3 py-2 text-xs rounded-xl border border-stone-200 bg-white focus:outline-none focus:ring-2 focus:ring-blue-400 text-stone-700"
            />

            <select
              value={recentMealSlot}
              onChange={(e) => setRecentMealSlot(e.target.value as MealSlot)}
              className="px-3 py-2 text-xs rounded-xl border border-stone-200 bg-white focus:outline-none focus:ring-2 focus:ring-blue-400 text-stone-700 font-medium"
            >
            ...
            <button
              type="submit"
              className="px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold transition-colors flex items-center gap-1 shadow-2xs"
            >
              <Plus className="w-3.5 h-3.5" />
              <span>기록</span>
            </button>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {currentUser.recentMeals.map((meal) => {
                ...
                return (
                  <div
                    key={meal.id}
                    className="p-3 rounded-2xl bg-white border border-stone-200 flex items-center justify-between shadow-2xs hover:border-stone-300 transition-colors"
                  >
